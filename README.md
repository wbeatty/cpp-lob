...
├── analysis/
│   └── data.ipynb            # latency analysis notebook (reads ../output/)
├── docs/img/                 # graphs used in this README
├── data/
│   ├── test1.txt             # functional scenario (~800 lines)
│   ├── test2.txt             # 1M orders
│   └── test3.txt             # 5M+ orders generated by -r
├── output/                   # latency telemetry emitted at run time
└── Makefile
```
...
```bash
# Small sample
./cpp-lob -f data/test1.txt

# Bundled scenarios
./cpp-lob -f data/test1.txt    # ~800 orders, mixed book dynamics
./cpp-lob -f data/test2.txt    # 1M orders
./cpp-lob -f data/test3.txt    # 5M+ realistic flow (generate with -r)

# Stream trade fills to stdout
./cpp-lob -f data/test1.txt -o
```
